...
"""
С помощью @login_required сделал функции доступными только авторизированным пользователям.

"""
#Проверяется валидность и привязывается автор из request.user
@login_required
...
#Если пользователь не существует, возвращает 404.
...
#Получает пост по post_id. Если пост не найден, возвращает 404.
@login_required
...
#Использует UserForm с текущими данными пользователя.
@login_required
...
#Использует PostForm с данными текущего поста.
...
#Получает пост по post_id и проверяет, что автор поста — текущий пользователь. Если это не так, возвращает 404.
@login_required
...
"""
Получает комментарий по comment_id, проверяет, 
что автор комментария — текущий пользователь 
и комментарий относится к указанному посту.
"""
@login_required
